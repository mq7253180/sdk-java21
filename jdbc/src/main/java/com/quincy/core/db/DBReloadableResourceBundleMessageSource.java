 ...
 *
 * <p>In contrast to the JDK-based {@link org.springframework.context.support.ResourceBundleMessageSource}, this class uses
 ...
 * {@link org.springframework.context.support.ResourceBundleMessageSource}. It follows the basic ResourceBundle rule of not
 ...
 */
public class DBReloadableResourceBundleMessageSource extends AbstractResourceBasedMessageSource implements ResourceLoaderAware {
	...
	protected PropertiesHolder getMergedProperties(Locale locale) {
		...
		this.loadFromDB(locale, mergedHolder);
	...
	/**
	 * Load i18n resource from db.
	 * @param locale
	 * @param mergedHolder
	 */
	private void loadFromDB(Locale locale, PropertiesHolder mergedHolder) {
		Connection conn = null;
		PreparedStatement stat = null;
		ResultSet rs = null;
		try {
			conn = this.dataSource.getConnection();
			stat = conn.prepareStatement("SELECT k.key,v.value FROM t_core_i18n_locale l INNER JOIN t_core_i18n_value v ON l.id=v.locale_id INNER JOIN t_core_i18n_key k ON v.key_id=k.id WHERE l.locale=?");
			stat.setString(1, locale.toLanguageTag());
			rs = stat.executeQuery();
			while(rs.next()) {
				String key = rs.getString("key");
				String value = rs.getString("value");
				mergedHolder.getProperties().put(key, value);
			}
		} catch (SQLException e) {
			logger.error("Loading i18n resource from db failed: ", e);
		} finally {
			try {
				if(rs!=null)
					rs.close();
				...
				logger.error("Releasing DB connection failed: ", e);
			}
		}
	}
